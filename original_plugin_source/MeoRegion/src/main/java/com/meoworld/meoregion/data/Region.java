...
public class Region {
    ...
    public Optional<SubChunk> getSubChunk(ChunkCoordinate coord) {
        return Optional.ofNullable(subChunks.get(coord));
    }

    public SubChunk getOrCreateSubChunk(ChunkCoordinate coord) {
        return subChunks.computeIfAbsent(coord, k -> new SubChunk());
    }
}
